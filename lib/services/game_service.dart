...
class GameService {
  ...
  static Future<List<Question>> getGameQuestions(gameMode, userRank, tags) async{
    var response = await http.post(Uri.parse('$baseUrl/games/play'), headers: headers,
    body: jsonEncode({'gameMode': gameMode, 'userRank': userRank, 'tags': tags})
    );
  ...
  static Future<List<Question>> getGameQuestionsWithoutToken(gameMode, userRank, tags) async{
    var response = await http.post(Uri.parse('$baseUrl/games/play/open'), headers: headers,
        body: jsonEncode({'gameMode': gameMode, 'userRank': userRank, 'tags': tags})
    );
    return questionFromJson(response.body);
  }
  ...
  static Future<void> sendGameData(gameMode, totalScore, baseScore, bonusScore, averageTimeSpent,
      playerRank, noOfCorrectAnswers, playerId, userProgress) async {
    var response = await http.post(Uri.parse('$baseUrl/playlog'), headers: headers,
        body: jsonEncode({'game_mode': gameMode, 'total_score': totalScore, 'base_score': baseScore,
         'bonus_score': bonusScore, 'average_time_spent': averageTimeSpent, 'player_rank': playerRank,
          'number_of_correct_answers': noOfCorrectAnswers, 'player_id': playerId, 'user_progress': userProgress}));
  }
